// Blink reminder — floating pill with slide-up entrance, floating bob, and slide-out exit.
...
fn applyPosition(y: CGFloat, alpha: CGFloat) void {
    if (blink_window != null) {
        appkit.setWindowFrame(blink_window, NSRect{
            .origin = NSPoint{ .x = screen_x, .y = y },
            .size = NSSize{ .width = window_width, .height = window_height },
        });
        appkit.setAlphaValue(blink_window, alpha);
    }
}
...
pub fn showBlinkReminder() void {
    ...
    if (config.gifString(&app_mod.state.blink_gif)) |gif_name| {
